...
//#endregion
//#region node_modules/photoswipe/dist/photoswipe.esm.js
/*!
...
var PanBounds = class {
	...
	*/
	_updateAxis(axis) {
		...
		const padding = parsePaddingOption$1(axis === "x" ? "left" : "top", pswp.options, pswp.viewportSize, this.slide.data, this.slide.index);
...
var ZoomLevel$1 = class {
	...
	*/
	_getSecondary() {
		...
		if (this.elementSize && currZoomLevel * this.elementSize.x > MAX_IMAGE_WIDTH$1) currZoomLevel = MAX_IMAGE_WIDTH$1 / this.elementSize.x;
	...
	*/
	...
	change() {
		...
		if (mainScroll.isShifted()) {
			...
			if (velocity.x < -.5 && currentSlideVisibilityRatio < 0 || velocity.x < .1 && currentSlideVisibilityRatio < -.5) {
				indexDiff = 1;
	...
	*/
	_finishPanGestureForAxis(axis) {
		...
		if (restoreBgOpacity) {
			...
			if (vDragRatio < 0 && projectedVDragRatio < -.4 || vDragRatio > 0 && projectedVDragRatio > MIN_RATIO_TO_CLOSE) {
				this.pswp.close();
				return;
			}
		}
...
var UIElement = class {
 ...
 /** @type {HTMLButtonElement} */
		...
		if (appendTo === "bar") {
			if (!pswp.topBar) pswp.topBar = createElement$1("pswp__top-bar pswp__hide-on-close", "div", pswp.scrollWrap);
...
var Eventable$1 = class {
	...
	*/
	dispatch(name, details) {
		...
		const event = new PhotoSwipeEvent$1(name, details);
...
var Placeholder$1 = class {
	...
	*/
	setDisplayedSize(width, height) {
		...
		} else setWidthHeight$1(this.element, width, height);
	}
	destroy() {
		var _this$element;
...
var Content$1 = class {
	...
	*/
	append() {
		...
		if (this.isImageContent()) if (supportsDecode && this.slide && (!this.slide.isActive || isSafari$1())) {
	...
	*/
	activate() {
		...
		if (this.isImageContent() && this.isDecoding && !isSafari$1()) this.appendImage();
...
function lazyLoadData$1(itemData, instance, index) {
	...
	if (options) {
		...
		else viewportSize = getViewportSize$1(options, instance);
...
*/
var PhotoSwipeBase$1 = class extends Eventable$1 {
	/**
	...
	*/
	_getGalleryDOMElements(galleryElement) {
		...
		if ((_this$options3 = this.options) !== null && _this$options3 !== void 0 && _this$options3.children || (_this$options4 = this.options) !== null && _this$options4 !== void 0 && _this$options4.childSelector) return getElementsFromOption$1(this.options.children, this.options.childSelector, galleryElement) || [];
...
* PhotoSwipe Core
*/
var PhotoSwipe = class extends PhotoSwipeBase$1 {
	/**
	* @param {PhotoSwipeOptions} [options]
	*/
	constructor(options) {
...
//#endregion
//#region node_modules/photoswipe/dist/photoswipe-lightbox.esm.js
/*!
...
var PhotoSwipeLightbox = class extends PhotoSwipeBase {
	...
	*/
	onThumbnailsClick(e) {
		...
		/** @type {DataSource} */
		const dataSource = { gallery: e.currentTarget };
		if (clickedIndex >= 0) {
			e.preventDefault();
	...
	*/
	getClickedIndex(e) {
		...
		const clickedChildIndex = getElementsFromOption(
			this.options.children,
			this.options.childSelector,
			/** @type {HTMLElement} */
			e.currentTarget
		).findIndex((child) => child === clickedTarget || child.contains(clickedTarget));
		if (clickedChildIndex !== -1) return clickedChildIndex;
		else if (this.options.children || this.options.childSelector) return -1;
		return 0;
	}
	/**
	...
	*/
	loadAndOpen(index, dataSource, initialPoint) {
		...
		if (!dataSource && this.options.gallery && this.options.children) {
			...
			if (galleryElements[0]) dataSource = { gallery: galleryElements[0] };
		}
	...
	*/
	preload(index, dataSource) {
		...
		if (isPswpClass(options.pswpModule)) promiseArray.push(Promise.resolve(
			/** @type {Type<PhotoSwipe>} */
			options.pswpModule
		));
		else if (pswpModuleType === "string") throw new Error("pswpModule as string is no longer supported");
		else if (pswpModuleType === "function") promiseArray.push(
			/** @type {() => Promise<Type<PhotoSwipe>>} */
			options.pswpModule()
		);
...
//#endregion
//#region node_modules/@vovayatsyuk/photoswipe-object-position/photoswipe-object-position.js
	...
	constructor(lightbox) {
		...
		*/
		lightbox.on("initialZoomPan", (event) => {
			...
			if (positionX !== "50%" && slide.pan.x < 0) slide.pan.x = getCroppedZoomPan(positionX, slide.bounds.min.x, slide.bounds.max.x);
			if (positionY !== "50%" && slide.pan.y < 0) slide.pan.y = getCroppedZoomPan(positionY, slide.bounds.min.y, slide.bounds.max.y);
		});
		/**
...
//#endregion
//#region .11ty-vite/src/screenshots.js
